...
class RunParameters {
 public:
  ...
  void setTotalNgroups(unsigned long n) {
    ...
    TotalNblocks = static_cast<unsigned long>(sqrt(0.25 + 2.0*Ng)-0.5 + 0.1); //  sqrt(1/4 + 2*Ng)-1/2);

  cerr << "n: " << n << " TotalNgroups: " << TotalNgroups << " Ng: " << Ng << " TotalNBlocks: " << TotalNblocks << endl;
  ...
  void checkNcharBase(DataParameters &dp){
    double p = dp.getNchar() / NcharBase - TotalNblocks+1;
	 cerr << " Nchar: " << dp.getNchar() << " NcharBase: " << NcharBase << " TotalNblocks: " << TotalNblocks << endl;
    if (p>1 || p<=0){
